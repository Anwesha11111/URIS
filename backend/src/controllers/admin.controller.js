...
async function getAdminOverview(req, res, next) {
  try {
    ...
    // Pagination — defaults to page 1, 50 interns per page
    ...
    // Phase 6: Only show interns whose user account is active in the operational overview.
    // Inactive/archived/removed users are excluded from active dashboards.
    internFilter = {
      ...internFilter,
      user: { status: 'active' },
    };
    ...
    return ok(res, { totalInterns, activeTasks, openAlerts, completedLast30, interns, alerts, teams, pagination: { page, limit, total: totalInterns, pages: Math.ceil(totalInterns / limit) } });
  } catch (err) {
    next(err);
  }
}
...
async function changeUserRole(req, res, next) {
  try {
    ...
    // Update role and record history in a transaction
