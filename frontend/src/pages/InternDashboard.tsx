          ...
          {!loading && !error && data && (
            <>
              ...
              {!data.isNewUser && (
                <>
              {/* ── Availability banner ───────────────────────────────────────
                  Shown when the intern has an availability_reminder alert (set
                  by the backend scheduler when no submission exists for the
                  current week) OR it is Monday and they have no active tasks
                  (strong signal they haven't submitted yet).                 */}
              {(() => {
                const hasReminderAlert = data.unreadAlerts?.some(
                  (a: { type: string }) => a.type === 'availability_reminder'
                )
