...
import { LayoutDashboard, CalendarDays, ClipboardList, Star, Users, Bell, LogOut, ChevronRight, ShieldCheck } from 'lucide-react'
      ...
      <div className="px-2">
        ...
        <div className="px-3 mb-3">
          ...
          <p className="nav-label text-[0.5rem] mt-0.5" style={{ color: 'rgba(201,168,76,0.45)' }}>
            {isAdmin() ? 'LEAD · FULL ACCESS' : 'INTERN · LIMITED'}
          </p>
        </div>
        <button onClick={() => { clearAuth(); nav('/login') }} className="sidebar-item w-full" style={{ color: 'rgba(248,113,113,0.5)' }}>
          <LogOut size={13} />
          Sign Out
        </button>
      </div>
